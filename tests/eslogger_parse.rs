...
fn parses_fork_fixture() {
    ...
    let ev = peekaboo::trace::eslogger::parse_line(raw).unwrap().expect("fork should parse");
    assert_eq!(ev.kind, peekaboo::event::EventKind::Fork);
    assert_eq!(ev.pid, 1234);
    match ev.data {
        peekaboo::event::EventData::Fork { child_pid } => {
