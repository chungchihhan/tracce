use tracce::event::{Event, EventData, EventKind, FileOp, NetProto, ProcessRef};
...
fn roundtrip_file_event_with_sensitive_flag() {
    let e = Event {
        ...
        flags: tracce::event::FLAG_SENSITIVE,
    };
    let line = serde_json::to_string(&e).unwrap();
    let back: Event = serde_json::from_str(&line).unwrap();
    assert_eq!(back.flags & tracce::event::FLAG_SENSITIVE, tracce::event::FLAG_SENSITIVE);
}
